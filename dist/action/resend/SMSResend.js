"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.SMSResend = void 0;
var Resend_1 = require("./Resend");
var SMSResend = /** @class */ (function (_super) {
    __extends(SMSResend, _super);
    function SMSResend() {
        return _super.call(this, "sms") || this;
    }
    SMSResend.prototype.message = function (message) {
        this.recontentDetail.message = message;
        return this;
    };
    SMSResend.prototype.validation = function (requestBody) {
        this.requiredFieldByType.push('recontent.sms.message');
        this.maxFiledByType.set('recontent.sms.message', 90);
        return _super.prototype.validation.call(this, requestBody);
    };
    return SMSResend;
}(Resend_1.Resend));
exports.SMSResend = SMSResend;
